document.addEventListener('DOMContentLoaded', function() {
    ...
    const chatArea = document.getElementById('chatArea'); // Added for browser panel layout
    ...
    function closeAllPanels() {
        ...
        chatArea.classList.remove('browser-open'); // Use the correct class for chatArea
    }
    ...
    browserPanelBtn.addEventListener('click', () => {
        closeAllPanels();
        chatArea.classList.add('browser-open'); // Add the correct class to chatArea
        browserPanel.classList.add('open'); // Explicitly open the browser panel
        sidebar.classList.remove('open'); // Close sidebar
        updateScreenshot();
    });

    closeBrowserPanelBtn.addEventListener('click', () => {
        chatArea.classList.remove('browser-open'); // Remove the correct class from chatArea
        browserPanel.classList.remove('open'); // Also close the panel itself
    });
    ...
    async function processUserMessage(message, chatHistory) {
        ...
        // --- Open Browser Panel Immediately if BrowserUse tool is determined ---
        if (tool === 'BrowserUse') {
            console.log("BrowserUse tool determined, opening panel immediately.");
            closeAllPanels(); // Close other panels first
            chatArea.classList.add('browser-open'); // Adjust chat area layout
            browserPanel.classList.add('open'); // Open the browser panel
            updateScreenshot(); // Load initial screenshot
        }
        // --- End Immediate Browser Panel Open ---
        ...
        try {
            ...
            if (data.success) {
                if (data.type === 'text') {
                   ...
                   // Add a unique ID to the image for later reference
                   const plotId = `plot-${Date.now()}-${Math.random().toString(36).substring(2, 9)}`;
                   const plotHtml = `
                       <div class="plot-container">
                           <img src="${plotUrl}" alt="Generated Plot for ${escapeHtml(file.name)}" class="plot-image" id="${plotId}">
                       </div>
                       <p>Generated plot for ${escapeHtml(file.name)}.</p>
                   `;
                    // Return the HTML content directly to be added to the chat
                    return { content: plotHtml, allowHtml: true }; // Signal to addMessage to allow HTML
                } else {
    ...
    function addMessage(content, sender, sources = [], allowHtml = false) {
        ...
        let messageContent;
        if (allowHtml) {
            // If allowHtml is true, use the content directly
            messageContent = content;
        } else {
            // Otherwise, escape and format the content as before
            messageContent = content
